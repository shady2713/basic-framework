...
class PreferenceManager {
  ...
  initPreferences = async ({ namespace, overrides }: InitialOptions) => {
    ...
    // 加载缓存的偏好设置并与初始配置合并
    // 优先级: 项目覆盖 > 缓存 > 默认配置
    // 注：项目覆盖优先于缓存，确保 accessMode 等关键配置不会被旧缓存覆盖
    ...
    // 版权设置允许用户通过 UI 修改，若缓存中有版权数据则优先使用缓存
    if (cachedPreferences?.copyright) {
      mergedPreference.copyright = cachedPreferences.copyright;
    }
