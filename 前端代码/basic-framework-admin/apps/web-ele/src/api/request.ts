...
import { retryAfterMfaStepUp } from '#/utils/mfa-step-up';
...
function createRequestClient(baseURL: string, options?: RequestClientOptions) {
  ...
  // 高风险操作要求二次验证时，全局完成挑战并且只重试原请求一次。
  client.addResponseInterceptor({
    rejected: (error) =>
      retryAfterMfaStepUp(error, (config) =>
        client.request(config.url as string, { ...config }),
      ),
  });
