...
import SmsTemplateIndex from './index.vue';
...
const state = vi.hoisted(() => ({
  ...
  row: {} as SystemSmsTemplateApi.Template,
}));
...
function mountPage() {
  return mount(SmsTemplateIndex, { attachTo: document.body });
}
...
describe('system sms template page', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    state.row = template();
    state.gridConfig = undefined;
    vi.mocked(getSmsTemplatePage).mockResolvedValue({
      list: [template()],
      total: 1,
    });
  });

  it('queries sms templates with paging and search values', async () => {
    mountPage();
    const query = state.gridConfig?.gridOptions.proxyConfig.ajax.query;

    await query?.({ page: { currentPage: 1, pageSize: 10 } }, { name: 'x' });

    expect(getSmsTemplatePage).toHaveBeenCalledWith({
      name: 'x',
      pageNo: 1,
      pageSize: 10,
    });
  ...
  it('creates, edits and deletes an sms template', async () => {
    ...
    await flushPromises();
    expect(deleteSmsTemplate).toHaveBeenCalledWith(1);
