...
const ENABLED_FILE_STORAGE_VALUES = new Set([10, 20]);

function getEnabledFileStorageOptions() {
  return getDictOptions(DICT_TYPE.INFRA_FILE_STORAGE, 'number').filter(
    (option) => ENABLED_FILE_STORAGE_VALUES.has(Number(option.value)),
  );
}
...
export function useFormSchema(): VbenFormSchema[] {
  return [
    ...
    },
    {
      ...
      component: 'Select',
      componentProps: {
        options: getEnabledFileStorageOptions(),
        placeholder: '请选择存储器',
      },
      rules: 'required',
      dependencies: {
        triggerFields: ['id'],
        disabled: (formValues) => Boolean(formValues.id),
      },
    },
    {
      fieldName: 'remark',
    ...
    // Local
    {
      ...
      dependencies: {
        ...
        show: (formValues) => formValues.storage === 10,
      },
    },
    // S3
    {
    ...
    },
    {
      ...
      dependencies: {
        triggerFields: ['id', 'storage'],
        show: (formValues) => formValues.storage === 20,
        rules: (values) =>
          values.id
            ? z.string().optional()
            : z.string().min(1, '请输入 accessSecret'),
      },
    },
    {
