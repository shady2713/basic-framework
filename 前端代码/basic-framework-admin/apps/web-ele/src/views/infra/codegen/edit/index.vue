...
import { ElButton, ElLoading, ElStep, ElSteps } from 'element-plus';
...
async function submitForm() {
  ...
  if (!basicInfoValid) {
    showWarningMessage('保存失败，原因：基本信息表单校验失败请检查');
    return;
  }
  const generateInfoValid = await generateInfoRef.value?.validate();
  if (!generateInfoValid) {
    showWarningMessage('保存失败，原因：生成信息表单校验失败请检查');
    return;
  }
