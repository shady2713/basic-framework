...
const DEFAULT_CONFIG = {
  ...
  ignoreDirs: [
    ...
    // 上游既有设计：实现文件与同包 types.ts 之间的类型级循环，
    // 不重构上游代码，按既有 ignoreDirs 机制登记豁免
    'packages/@core/ui-kit/form-ui/src/', // form-api.ts ↔ types.ts
    'packages/effects/plugins/src/vxe-table/', // api.ts ↔ types.ts
  ],
  threshold: 0, // 循环依赖的阈值
} as const;
