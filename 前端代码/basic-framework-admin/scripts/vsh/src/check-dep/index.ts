  ...
  try {
    ...
    await Promise.all(
      packages.map(async (pkg: PackageInfo) => {
        ...
        // 包以自身包名做导入（Node exports self-reference，如
        // @vben-core/typings 的 vue-router.d.ts 模块增强）必然可解析，
        // 将包自身名加入本包的 ignoreMatches 以消除自引用误报
        const unused = await depcheck(pkg.dir, {
          ignoreMatches: [...finalConfig.ignoreMatches, pkg.packageJson.name],
