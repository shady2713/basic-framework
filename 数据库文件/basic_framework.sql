...
-- Snapshot note: generated from the authoritative Flyway V1-V25 migration chain.
-- Only the 19 soft-delete tables retain a deleted column; hard-delete and
...
CREATE TABLE `infra_codegen_column` (
  ...
  CONSTRAINT `fk_codegen_column_table` FOREIGN KEY (`table_id`) REFERENCES `infra_codegen_table` (`id`) ON DELETE CASCADE
...
CREATE TABLE `infra_codegen_table` (
  ...
  KEY `idx_sub_join_column_id` (`sub_join_column_id`),
  KEY `idx_tree_parent_column_id` (`tree_parent_column_id`),
  KEY `idx_tree_name_column_id` (`tree_name_column_id`),
  KEY `idx_parent_menu_id` (`parent_menu_id`),
  CONSTRAINT `chk_codegen_scene_parent_menu_shape` CHECK (((`deleted` = 0x01) or ((`scene` = 1) and ((`parent_menu_id` is null) or (`parent_menu_id` >= 0))) or ((`scene` = 2) and (`parent_menu_id` is null)))),
  CONSTRAINT `chk_codegen_sub_reference_shape` CHECK (((`deleted` = 0x01) or ((`template_type` = 15) and (`master_table_id` is not null) and (`sub_join_column_id` is not null) and (`sub_join_many` is not null)) or ((`template_type` <> 15) and (`master_table_id` is null) and (`sub_join_column_id` is null) and (`sub_join_many` is null)))),
  CONSTRAINT `chk_codegen_template_type` CHECK (((`deleted` = 0x01) or (`template_type` in (1,2,10,11,12,15)))),
  CONSTRAINT `chk_codegen_tree_reference_shape` CHECK (((`deleted` = 0x01) or ((`template_type` = 2) and (`tree_parent_column_id` is not null) and (`tree_name_column_id` is not null) and (`tree_parent_column_id` <> `tree_name_column_id`)) or ((`template_type` <> 2) and (`tree_parent_column_id` is null) and (`tree_name_column_id` is null))))
...
INSERT INTO `infra_job` VALUES (25,'访问日志清理 Job',1,'accessLogCleanJob','','0 0 0 * * ?',3,0,0,'1','2023-10-03 10:59:41','1','2026-08-23 00:00:00',_binary '\0'),(26,'错误日志清理 Job',1,'errorLogCleanJob','','0 0 0 * * ?',3,0,0,'1','2023-10-03 11:00:43','1','2026-08-23 00:00:00',_binary '\0'),(27,'任务日志清理 Job',1,'jobLogCleanJob','','0 0 0 * * ?',3,0,0,'1','2023-10-03 11:01:33','1','2026-08-23 00:00:00',_binary '\0'),(28,'system 数据保留清理 Job',1,'systemDataRetentionCleanJob','','0 0 1 * * ?',3,0,0,'1','2026-08-23 00:00:00','1','2026-08-23 00:00:00',_binary '\0'),(29,'system 数据完整性审计 Job',1,'systemDataIntegrityAuditJob','','0 30 2 * * ?',0,0,0,'1','2026-08-23 00:00:00','1','2026-08-23 00:00:00',_binary '\0'),(30,'infra 数据完整性审计 Job',1,'infraDataIntegrityAuditJob','','0 45 2 * * ?',0,0,0,'1','2026-08-24 00:00:00','1','2026-08-24 00:00:00',_binary '\0');
