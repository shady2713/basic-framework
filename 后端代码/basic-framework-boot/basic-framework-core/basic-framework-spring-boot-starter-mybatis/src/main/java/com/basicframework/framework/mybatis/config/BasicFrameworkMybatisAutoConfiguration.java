 ...
 */
@AutoConfiguration(
        before = MybatisPlusAutoConfiguration.class) // 目的：先于 MyBatis Plus 自动配置，避免 @MapperScan 可能扫描不到 Mapper 打印 warn 日志
@MapperScan(
        value = "${basic-framework.info.base-package}",
...
public class BasicFrameworkMybatisAutoConfiguration {
    ...
    @Bean
    public MetaObjectHandler defaultMetaObjectHandler(ObjectProvider<CurrentUserProvider> currentUserProvider) {
        return new DefaultDBFieldHandler(currentUserProvider); // 自动填充参数类
    }
