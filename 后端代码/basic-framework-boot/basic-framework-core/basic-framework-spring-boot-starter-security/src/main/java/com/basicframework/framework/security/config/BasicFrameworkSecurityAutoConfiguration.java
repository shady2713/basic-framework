...
import com.basicframework.framework.security.core.aop.MfaStepUpAspect;
...
import com.basicframework.framework.security.core.service.CurrentUserProviderImpl;
...
public class BasicFrameworkSecurityAutoConfiguration {
    ...
    public CredentialCipher credentialCipher() {
        return new CredentialCipher(securityProperties);
    }
    ...
    /** 高风险操作统一 MFA 二次验证接缝。 */
    @Bean
    public MfaStepUpAspect mfaStepUpAspect(MfaCommonApi mfaApi) {
        return new MfaStepUpAspect(securityProperties, mfaApi);
    }

    /**
     * {@link CurrentUserProvider} 能力接缝的实现 Bean，
     * 供 mybatis 字段填充、数据权限等 starter 经 ObjectProvider 消费
     */
    @Bean
