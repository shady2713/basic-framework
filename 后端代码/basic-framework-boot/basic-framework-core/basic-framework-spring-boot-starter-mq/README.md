...
- Stream 的无效 JSON 与 JSON `null` 内容均按不可重试错误进入 DLQ；Pub/Sub 的 JSON `null` 内容会在拦截器和业务监听器之前被拒绝，避免空消息进入业务代码。
...
- 消费、重投和清理失败只记录脱敏后的有界异常堆栈；不得把捕获的异常对象或异常消息直接交给日志框架。
...
- Stream 消费者组在应用启动时串行注册；创建失败后必须回读 Redis 组元数据，只有目标组确实存在才继续启动。连接、权限、命令或状态回读错误会携带 Stream/组上下文使启动失败，不解析驱动异常文案决定状态。
- 未显式传入组名的 Stream 监听器由 Consumer 自动配置使用必填的 `spring.application.name` 初始化；该默认值只在自动配置边界解析，不注入抽象业务监听器字段。显式组名优先，空应用名在启动期失败。
- Stream 历史清理会检查同一 Stream 的全部消费组，只删除早于“最近保留边界、各组最早 pending/最后投递边界”中最早 ID 的记录。因此 pending 和尚未投递的消息优先于长度上限保留，ACK 后才会进入后续清理。
- 可靠性配置统一位于 `basic-framework.mq.redis.*`：`stream-read-batch-size=10`（允许 1–1000）、`max-delivery-attempts=5`、`retry-jitter-factor=0.2`、`retry-max-delay=1h`、`dead-letter-audit-retention=30d`；历史配置 `stream-max-length=10000`、`stream-cleanup-batch-size=10000` 保持不变。非法边界在启动期失败。
...
通过 Maven `integration` profile 运行；该 profile 的 JaCoCo 行覆盖率棘轮为 91.4%。
