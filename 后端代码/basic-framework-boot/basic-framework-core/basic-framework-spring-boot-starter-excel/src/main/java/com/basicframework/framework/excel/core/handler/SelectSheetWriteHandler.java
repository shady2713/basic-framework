...
public class SelectSheetWriteHandler implements SheetWriteHandler {
    ...
    private void getSelectDataList(int colIndex, Field field) {
        ...
        ExcelColumnSelectFunction function =
    ...
    public void afterSheetCreate(WriteWorkbookHolder writeWorkbookHolder, WriteSheetHolder writeSheetHolder) {
        ...
        List<KeyValue<Integer, List<String>>> keyValues =
                convertList(selectMap.entrySet(), entry -> new KeyValue<>(entry.getKey(), entry.getValue()));
        ...
        for (KeyValue<Integer, List<String>> keyValue : keyValues) {
            ...
            for (int i = 0; i < rowLength; i++) {
                ...
                row.createCell(keyValue.getKey())
                        .setCellValue(keyValue.getValue().get(i));
            }
            // 2.2 设置单元格下拉选择
     ...
     * 设置单元格下拉选择
     */
    private static void setColumnSelect(
        ...
        String refers = DICT_SHEET_NAME + "!$" + excelColumn + "$1:$" + excelColumn + "$"
                + keyValue.getValue().size();
        ...
        CellRangeAddressList rangeAddressList =
                new CellRangeAddressList(FIRST_ROW, LAST_ROW, keyValue.getKey(), keyValue.getKey());
