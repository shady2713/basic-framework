...
public class TencentSmsClient extends AbstractSmsClient {
    ...
    public SmsSendRespDTO sendSms(
        ...
        return new SmsSendRespDTO()
                .setSuccess(Objects.equals(API_CODE_SUCCESS, sendResult.getStr("Code")))
                .setApiRequestId(responseResult.getStr("RequestId"))
    ...
    public SmsTemplateRespDTO getSmsTemplate(String apiTemplateId) throws Throwable {
        ...
        JSONObject statusResult = response.getJSONObject("Response")
                .getJSONArray("DescribeTemplateStatusSet")
                .getJSONObject(0);
    ...
    private JSONObject request(String action, TreeMap<String, Object> body) {
        ...
        Date now = new Date();
        String nowStr = FastDateFormat.getInstance("yyyy-MM-dd", TimeZone.getTimeZone("UTC"))
                .format(now);
        ...
        byte[] secretService =
                hmac256(hmac256(("TC3" + properties.getApiSecret()).getBytes(StandardCharsets.UTF_8), nowStr), "sms");
        String signature = HexUtil.encodeHexStr(hmac256(hmac256(secretService, "tc3_request"), stringToSign));
        headers.put(
                "Authorization",
                "TC3-HMAC-SHA256" + " " + "Credential=" + getApiKey() + "/" + credentialScope + ", " + "SignedHeaders="
