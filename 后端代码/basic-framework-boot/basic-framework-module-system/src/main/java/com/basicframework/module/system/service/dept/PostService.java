package com.basicframework.module.system.service.dept;

import com.basicframework.framework.common.pojo.PageParam;
import com.basicframework.framework.common.pojo.PageResult;
import com.basicframework.module.system.dal.dataobject.dept.PostDO;
import java.util.Collection;
import java.util.List;
import org.springframework.lang.Nullable;

/**
 * 岗位 Service 接口
 *
 */
public interface PostService {

    /**
     * 创建岗位
     *
     * @param post 岗位信息
     * @return 岗位编号
     */
    Long createPost(PostDO post);

    /**
     * 更新岗位
     *
     * @param updateObj 岗位信息
     */
    void updatePost(PostDO updateObj);

    /**
     * 删除岗位信息
     *
     * @param id 岗位编号
     */
    void deletePost(Long id);

    /**
     * 批量删除岗位信息
     *
     * @param ids 岗位编号数组
     */
    void deletePostList(List<Long> ids);

    /**
     * 获得岗位列表
     *
     * @param ids 岗位编号数组
     * @return 部门列表
     */
    List<PostDO> getPostList(@Nullable Collection<Long> ids);

    /**
     * 获得符合条件的岗位列表
     *
     * @param ids 岗位编号数组。如果为空，不进行筛选
     * @param statuses 状态数组。如果为空，不进行筛选
     * @return 部门列表
     */
    List<PostDO> getPostList(@Nullable Collection<Long> ids, @Nullable Collection<Integer> statuses);

    /**
     * 获得岗位分页列表
     *
     * @param pageParam 分页参数
     * @param code      岗位编码，模糊匹配
     * @param name      岗位名称，模糊匹配
     * @param status    展示状态
     * @return 部门分页列表
     */
    PageResult<PostDO> getPostPage(PageParam pageParam, String code, String name, Integer status);

    /**
     * 获得岗位信息
     *
     * @param id 岗位编号
     * @return 岗位信息
     */
    PostDO getPost(Long id);

    /**
     * 校验岗位们是否有效。如下情况，视为无效：
     * 1. 岗位编号不存在
     * 2. 岗位被禁用
     *
     * @param ids 岗位编号数组
     */
    void validatePostList(Collection<Long> ids);
}
