...
import com.basicframework.framework.common.util.collection.MapUtils;
...
public class AliyunSmsClient extends AbstractSmsClient {
    ...
    public SmsTemplateRespDTO getSmsTemplate(String apiTemplateId) throws Throwable {
        ...
        if (ObjectUtil.notEqual(code, RESPONSE_CODE_SUCCESS)) {
            log.error(
                    "[getSmsTemplate][templateCode({}) responseCode({}) requestId({}) templateStatus({}) 响应不正确]",
                    apiTemplateId,
                    response.getStr("Code"),
                    response.getStr("RequestId"),
                    response.getInt("TemplateStatus"));
            return null;
        }
        // 2.2 请求成功
    ...
    private JSONObject request(String apiName, TreeMap<String, Object> queryParams) {
        ...
        headers.put(
                "x-acs-date",
                FastDateFormat.getInstance("yyyy-MM-dd'T'HH:mm:ss'Z'", TimeZone.getTimeZone("GMT"))
                        .format(new Date()));
        ...
        headers.entrySet().stream()
                ...
                .forEach(entry -> {
                    ...
                    canonicalHeaders
                            .append(lowerKey)
                            .append(":")
                            .append(String.valueOf(entry.getValue()).trim())
                            .append("\n");
        ...
        headers.put(
                "Authorization",
                "ACS3-HMAC-SHA256" + " " + "Credential=" + properties.getApiKey() + ", " + "SignedHeaders="
