package com.basicframework.module.infra.controller.admin.codegen;

import static com.basicframework.framework.common.pojo.CommonResult.success;
import static com.basicframework.framework.security.core.util.SecurityFrameworkUtils.getLoginUserNickname;
...
import com.basicframework.module.infra.controller.admin.codegen.vo.CodegenCreateListReqVO;
...
import com.basicframework.module.infra.controller.admin.codegen.vo.CodegenUpdateReqVO;
import com.basicframework.module.infra.controller.admin.codegen.vo.table.CodegenTablePageReqVO;
import com.basicframework.module.infra.controller.admin.codegen.vo.table.CodegenTableRespVO;
import com.basicframework.module.infra.controller.admin.codegen.vo.table.DatabaseTableRespVO;
import com.basicframework.module.infra.convert.codegen.CodegenConvert;
...
public class CodegenController {
    ...
    @Parameters({
        @Parameter(name = "name", description = "表名，模糊匹配", example = "sys_user"),
        @Parameter(name = "comment", description = "描述，模糊匹配", example = "用户表")
    })
    @PreAuthorize("@ss.hasPermission('infra:codegen:query')")
    public CommonResult<List<DatabaseTableRespVO>> getDatabaseTableList(
            @RequestParam(value = "name", required = false) String name,
            @RequestParam(value = "comment", required = false) String comment) {
        List<TableInfo> tables = codegenService.getDatabaseTableList(name, comment);
        return success(BeanUtils.toBean(tables, DatabaseTableRespVO.class));
    }
    ...
    public CommonResult<List<CodegenTableRespVO>> getCodegenTableList() {
        List<CodegenTableDO> list = codegenService.getCodegenTableList();
    ...
    public CommonResult<PageResult<CodegenTableRespVO>> getCodegenTablePage(@Valid CodegenTablePageReqVO pageReqVO) {
        PageResult<CodegenTableDO> pageResult = codegenService.getCodegenTablePage(
                pageReqVO,
                pageReqVO.getTableName(),
                pageReqVO.getTableComment(),
                pageReqVO.getClassName(),
    ...
    public CommonResult<List<Long>> createCodegenList(@Valid @RequestBody CodegenCreateListReqVO reqVO) {
        return success(codegenService.createCodegenList(getLoginUserNickname(), reqVO.getTableNames()));
    }
    ...
    public CommonResult<Boolean> updateCodegen(@Valid @RequestBody CodegenUpdateReqVO updateReqVO) {
        codegenService.updateCodegen(
                CodegenConvert.INSTANCE.convert(updateReqVO.getTable()),
                CodegenConvert.INSTANCE.convertSaveList(updateReqVO.getColumns()));
        return success(true);
    }
