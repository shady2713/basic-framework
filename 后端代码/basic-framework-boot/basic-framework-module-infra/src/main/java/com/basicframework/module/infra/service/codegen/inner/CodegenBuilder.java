...
import com.basicframework.module.infra.enums.codegen.CodegenColumnHtmlTypeEnum;
import com.basicframework.module.infra.enums.codegen.CodegenColumnListConditionEnum;
...
public class CodegenBuilder {
    ...
    /**
     * {@link com.basicframework.framework.mybatis.core.dataobject.BaseDO} 的字段
     */
    ...
    private void processColumnUI(CodegenColumnDO column) {
        ...
        COLUMN_HTML_TYPE_MAPPINGS.entrySet().stream()
                .filter(entry -> StrUtil.endWithIgnoreCase(column.getJavaField(), entry.getKey()))
                .findFirst()
                .ifPresent(entry -> column.setHtmlType(entry.getValue().getType()));
